...
public class ProgramClientTestRun extends ClientTestBase {
  ...
  public void testBatchProgramCalls() throws Exception {
    ...
    try {
      ...
      List<BatchProgram> programs = ImmutableList.of(flow, service);
      programClient.waitForStatus(namespace, programs, "RUNNING", 2, TimeUnit.MINUTES);

      // make a batch call for status of programs, one of which does not exist
      programs = ImmutableList.of(flow, service, missing);
      ...
      // wait for programs to enter stopped state
      programs = ImmutableList.of(flow, service);
      programClient.waitForStatus(namespace, programs, "STOPPED", 2, TimeUnit.MINUTES);
