...
public class ProgramLifecycleHttpHandler extends AbstractAppFabricHttpHandler {
  ...
  public void programHistory(HttpRequest request, HttpResponder responder,
    ...
    if (type == null || type == ProgramType.WEBAPP) {
      throw new MethodNotAllowed(String.format("Program history is not supported for program type '%s'.", programType));
    }
  ...
  public void programRunRecord(HttpRequest request, HttpResponder responder,
                             ...
                             @PathParam("program-type") String programType,
                             @PathParam("program-id") String programId,
                             @PathParam("run-id") String runid) throws NotFoundException, MethodNotAllowed {
    ProgramType type = getProgramType(programType);
    if (type == null || type == ProgramType.WEBAPP) {
      throw new MethodNotAllowed(String.format("Program run record is not supported for program " +
                                                 "type '%s'.", programType));
    }
  ...
  public void getProgramRuntimeArgs(HttpRequest request, HttpResponder responder,
    ...
    NotImplementedException, NotFoundException, MethodNotAllowed {
    ProgramType type = getProgramType(programType);
    if (type == null || type == ProgramType.WEBAPP) {
      throw new MethodNotAllowed(String.format("Get program runtime arguments is not supported for program " +
                                                 "type '%s'.", programType));
    }
  ...
  public void saveProgramRuntimeArgs(HttpRequest request, HttpResponder responder,
    ...
    ProgramType type = getProgramType(programType);
    if (type == null || type == ProgramType.WEBAPP) {
      throw new MethodNotAllowed(String.format("Save program runtime arguments is not supported for program " +
                                                 "type '%s'.", programType));
    }
  ...
  public void programSpecification(HttpRequest request, HttpResponder responder,
                                   ...
                                   @PathParam("program-id") String programId) throws NotFoundException,
                                                        MethodNotAllowed, BadRequestException, NotImplementedException {
    ProgramType type = getProgramType(programType);
    if (type == null) {
      throw new MethodNotAllowed(String.format("Program Specification is not supported for program " +
                                                 "type '%s'.", programType));
    }
